...
def subtract_files(a: Path, b: Path, c: Path, nomvar="ETAS", typvar="P@"):
    """
...
def remove_tides(fst_src: Path, fst_tides: Path, tides_member="000", fst_dst: Path = None):
    """
    ...
    for src in fst_src.iterdir():
        ...
        if dst.exists():
            ...
            continue
        subtract_files(src, tides_file, dst, nomvar="ETAS", typvar="P@")
        print(f"{src} => {dst} .. ok!")
...
def main():
    ...
    to_remove_tides = [
        root_dir / "prog_surge_tides",
        root_dir / "prog_surge_tides0"
    ]
    tides = root_dir / "prog_tides"

    tides_member = "000"

    for src in to_remove_tides:
        remove_tides(fst_src=src, fst_tides=tides, tides_member=tides_member)
